export const reduxDataStructure = function(treeStructure) {
    ...
    while (arr.length) {
        ...
        if (node.id !== 'rootNode') {
            allQuestionsOrder.push(node.id);
        }
        if (node.id !== 'rootNode' && node.parent.id === 'rootNode') {
            rootQuestionsOrder.push(node.id);
        }
    ...
    allQuestionsOrder.forEach(questionId => {
        if (
            formObject[questionId].conditionType !== 'noCondition' &&
            formObject[questionId].parentId !== 'rootNode'
        ) {
